...
Plugin URI: https://github.com/mizanur75/Wordpress_crud.git
Description: A Plugin For WordPress CRUD ( Create, Read, Update & Delete ) Application Using Ajax & WP List Table
Author: Mizannur Rahman
Author URI: https://github.com/mizanur75
Version: 1.0.0
*/
...
class EntryListTable extends WP_List_Table {
    ...
    function prepare_items() {
      ...
      $order = (isset($_REQUEST['order']) && in_array($_REQUEST['order'], array('asc', 'desc'))) ? $_REQUEST['order'] : 'desc';
        ...
        $this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE `name` LIKE '%".$_REQUEST['s']."%' OR `phone` LIKE '%".$_REQUEST['s']."%' OR `email` LIKE '%".$_REQUEST['s']."%' ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged * $per_page), ARRAY_A);
		  } else {
